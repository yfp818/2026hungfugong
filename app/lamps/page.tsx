...
export default function LampsPage() {
  ...
  const handleContactChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedId === "new") {
      ...
      if (contact) {
        setTargetName(contact.contact_name || ""); setBirthDate(contact.birth_date || ""); setAddress(contact.address || "");
      }
    }
  };
  ...
  const handleAddToCart = async (e: React.FormEvent) => {
    ...
    if (typeof addToCart === "function") {
      await addToCart({
        id: safeId, serviceType: "lamp", userName: finalTargetName, userPhone: phone,
        birthDate: birthDate, address: address, itemDetails: finalDetails, price: totalPrice 
      }, saveToContacts);
    }
          ...
          <form onSubmit={handleAddToCart} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                {products.map((p) => {
                  ...
                  return (
                    <div key={p.id} className={`p-5 rounded-2xl border-2 transition-all flex flex-col justify-between bg-card ${qty > 0 ? 'border-[#A61D24] dark:border-red-400 bg-red-50 dark:bg-red-900/20 shadow-md' : 'border-border hover:border-[#D89F3C] shadow-sm'}`}>
                      <div>
                        <div className="flex justify-between items-center mb-2"><h4 className="font-bold text-lg text-foreground">{p.title}</h4></div>
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-stone-100 dark:border-stone-800">
                        ...
                        <div className="flex items-center gap-2 bg-muted rounded-full border border-border p-1 shadow-sm">
                          <button type="button" onClick={() => handleQtyChange(p.id, -1)} disabled={qty === 0} className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-base transition-all ${qty > 0 ? 'bg-card text-stone-600 dark:text-stone-300 shadow-sm hover:text-[#A61D24] dark:hover:text-red-400' : 'text-stone-300 cursor-not-allowed'}`}>-</button>
                          <span className="w-4 text-center font-bold text-foreground text-sm">{qty}</span>
                          <button type="button" onClick={() => handleQtyChange(p.id, 1)} className="w-7 h-7 rounded-full bg-card shadow-sm flex items-center justify-center text-stone-600 dark:text-stone-300 font-bold text-base hover:text-[#A61D24] dark:hover:text-red-400 transition-all">+</button>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="lg:col-span-7 bg-card p-6 md:p-8 rounded-[2rem] border border-border shadow-sm space-y-6 h-fit sticky top-24">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select value={selectedContactId} onChange={handleContactChange} className="border border-stone-300 dark:border-stone-700 rounded-lg px-3 py-2 text-sm font-bold text-[#1A432D] dark:text-emerald-400 outline-none focus:border-[#A61D24] bg-muted">
                    ...
                    {safeContacts.length > 0 && <optgroup label="已儲存親友">{safeContacts.map((c: any) => <option key={c.id} value={c.id}>{c.contact_name}</option>)}</optgroup>}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="space-y-1.5"><label className="text-xs font-bold text-muted-foreground tracking-widest ml-1">聯絡人姓名</label><input required value={name} onChange={e=>setName(e.target.value)} placeholder="請填寫聯絡人" className="w-full bg-background border-2 border-border p-3.5 rounded-xl outline-none focus:border-[#A61D24] font-bold text-foreground placeholder:text-stone-400 transition-all"/></div>
                <div className="space-y-1.5"><label className="text-xs font-bold text-muted-foreground tracking-widest ml-1">祈福對象 <span className="text-muted-foreground font-normal">(選填)</span></label><input value={targetName} onChange={e=>setTargetName(e.target.value)} placeholder="若同聯絡人請留白" className="w-full bg-background border-2 border-border p-3.5 rounded-xl outline-none focus:border-[#A61D24] font-bold text-foreground placeholder:text-stone-400 transition-all"/></div>
                <div className="space-y-1.5"><label className="text-xs font-bold text-muted-foreground tracking-widest ml-1">聯絡電話</label><input required type="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="請輸入電話" className="w-full bg-background border-2 border-border p-3.5 rounded-xl outline-none focus:border-[#A61D24] font-bold text-foreground placeholder:text-stone-400 transition-all"/></div>
                <div className="space-y-1.5"><label className="text-xs font-bold text-muted-foreground tracking-widest ml-1">出生年月日</label><input required value={birthDate} onChange={e=>setBirthDate(e.target.value)} placeholder="例：1990/01/01" className="w-full bg-background border-2 border-border p-3.5 rounded-xl outline-none focus:border-[#A61D24] font-bold text-foreground placeholder:text-stone-400 transition-all"/></div>
                <div className="sm:col-span-2 space-y-1.5"><label className="text-xs font-bold text-muted-foreground tracking-widest ml-1">居住地址</label><input required value={address} onChange={e=>setAddress(e.target.value)} placeholder="請輸入完整地址" className="w-full bg-background border-2 border-border p-3.5 rounded-xl outline-none focus:border-[#A61D24] font-bold text-foreground placeholder:text-stone-400 transition-all"/></div>
                <div className="sm:col-span-2 space-y-1.5"><label className="text-xs font-bold text-muted-foreground tracking-widest ml-1">祈福心願 / 備註留言 <span className="text-muted-foreground font-normal">(選填)</span></label><textarea value={memo} onChange={e=>setMemo(e.target.value)} placeholder="例如：祈求玉帝保佑家人平安健康" rows={2} className="w-full bg-background border-2 border-border p-3.5 rounded-xl outline-none focus:border-[#A61D24] font-bold text-foreground placeholder:text-stone-400 transition-all resize-none"></textarea></div>
              </div>
              <label className="flex items-center gap-3 cursor-pointer w-max"><input type="checkbox" checked={saveToContacts} onChange={e=>setSaveToContacts(e.target.checked)} className="w-4 h-4 accent-[#A61D24]"/><span className="text-sm font-bold text-muted-foreground tracking-widest">同步儲存此對象至我的常用名冊</span></label>
              <div className="bg-muted border border-border p-5 md:p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-5 mt-4">
                <div><span className="text-sm text-muted-foreground font-bold tracking-widest">本次點燈小計</span><p className="text-3xl font-bold text-[#A61D24] mt-1">${totalPrice}</p></div>
                <Button type="submit" className="w-full sm:w-auto bg-[#1A432D] hover:bg-[#122F20] dark:bg-emerald-800 dark:hover:bg-emerald-900 text-white py-7 px-8 rounded-xl font-bold tracking-widest shadow-lg text-lg">加入點燈清單</Button>
              </div>
            </div>
          </form>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm transition-all">
            <div className="bg-background border-2 border-white/20 p-8 rounded-[2rem] max-w-sm w-full shadow-2xl space-y-8 text-center animate-in zoom-in-95 duration-300 relative overflow-hidden">
              ...
              <div className="flex flex-col gap-3 relative z-10 pt-2">
                <button onClick={() => router.push("/burning")} className="w-full flex items-center justify-center gap-2 bg-card hover:bg-muted border border-border text-foreground py-4 rounded-xl font-bold tracking-widest shadow-sm">
                ...
                <button onClick={() => router.push("/cart")} className="w-full bg-[#1A432D] hover:bg-[#122F20] dark:bg-emerald-800 dark:hover:bg-emerald-900 text-white py-4 rounded-xl font-bold tracking-widest shadow-lg mt-2">前往結帳對帳 →</button>
              </div>
            </div>
          </div>
        )}
