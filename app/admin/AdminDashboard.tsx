...
export default function AdminDashboard() {
  ...
  const handleAdminRefund = async () => {
    ...
    try {
      ...
      alert("功功德金發放成功。"); setRefundAmount(""); setRefundDesc(""); setSelectedMemberId(""); fetchData();
          ...
          <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section className="bg-card text-card-foreground p-6 md:p-8 rounded-[2rem] shadow-sm border border-border space-y-6">
              ...
              <div className="bg-muted text-muted-foreground p-6 rounded-2xl border border-border space-y-4">
                ...
                <div><input value={addNewsActionUrl} onChange={e => setAddNewsActionUrl(e.target.value)} placeholder="專屬活動報名連結 (選填)" className="w-full bg-background text-foreground border border-border p-3 rounded-xl outline-none"/></div>
              ...
              <div className="space-y-3">
                {newsList.map((n: any) => (
                  <div key={n.id} className="flex flex-col md:flex-row md:items-center justify-between border border-border p-4 rounded-xl bg-card text-card-foreground">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button onClick={()=>{ setEditNewsId(n.id); setAddNewsTitle(n.title); setAddNewsContent(n.content); setAddNewsPreviewUrl(n.image_url); setAddNewsCategory(n.category || 'news'); setAddNewsActionUrl(n.action_url || "");}} className="px-4 py-2 border border-border rounded-lg text-sm font-bold text-foreground hover:bg-muted">編輯</button>
          ...
          <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {servicesList.map((s: any) => (
                  <div key={s.id} className="flex justify-between items-center border border-border p-4 rounded-xl bg-card">
                    ...
                    <div className="flex gap-2">
                      <button onClick={()=>{setEditSrvId(s.id); setSrvTitle(s.title); setSrvLink(s.link_url)}} className="px-4 py-2 border border-border rounded-lg text-sm font-bold hover:bg-muted">編輯</button>
            ...
            <section className="bg-card text-card-foreground p-6 md:p-8 rounded-[2rem] shadow-sm border border-border space-y-6">
              ...
              <div className="bg-muted text-muted-foreground p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-6 border border-border">
                ...
                <div><label className="block text-xs font-bold text-muted-foreground mb-2">結帳金額</label><input type="number" value={pPrice} onChange={e=>setPPrice(Number(e.target.value))} className="w-full bg-background text-foreground border border-border p-3 rounded-xl outline-none"/></div>
            ...
            <section className="bg-card text-card-foreground p-6 md:p-8 rounded-[2rem] shadow-sm border border-border space-y-6">
              ...
              <div className="bg-muted text-muted-foreground p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-6 border border-border">
                ...
                <div className="md:col-span-2 flex gap-3">
                  <button onClick={handleAddFlashCampaign} className="flex-1 bg-purple-900 text-white py-4 rounded-xl font-bold tracking-widest text-sm shadow-sm">{editFcId ? "儲存活動更新" : "發布並推播至首頁焦點"}</button>
                  {editFcId && <button onClick={() => { setEditFcId(null); setFcTitle(""); setFcDesc(""); setFcPreviewUrl(""); setFcOptions([{ title: "", price: 0 }]); setFcImageFile(null); setFcSplash(false); }} className="px-6 bg-muted text-muted-foreground py-4 rounded-xl font-bold tracking-widest text-sm shadow-sm">取消</button>}
                </div>
              </div>
              <div className="space-y-4">
                {flashCampaignsList.map((fc: any) => (
                  <div key={fc.id} className="p-5 bg-card border border-border rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 justify-end w-full md:w-auto">
                      ...
                      <button onClick={() => {setEditFcId(fc.id); setFcTitle(fc.title); setFcDesc(fc.description || ""); setFcPreviewUrl(fc.image_url || ""); setFcSplash(fc.show_splash || false); setFcBankId(fc.bank_account_id || ""); setFcOptions(fc.options || [{title: "", price: fc.price || 0}]);}} className="text-xs font-bold bg-muted text-foreground px-4 py-2 rounded-lg">編輯</button>
                      <button onClick={async()=>{if(confirm("確定刪除此活動？")){await supabase.from("flash_campaigns").delete().eq("id", fc.id); fetchData();}}} className="text-xs font-bold bg-red-50 text-red-600 px-4 py-2 rounded-lg">刪除</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-card text-card-foreground p-6 md:p-8 rounded-[2rem] shadow-sm border border-border space-y-6">
              ...
              <div className="bg-muted text-muted-foreground p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-6 border border-border">
                <div><label className="block text-xs font-bold text-muted-foreground mb-2">獨立專案主標題</label><input value={spTitle} onChange={e=>setSpTitle(e.target.value)} placeholder="例: 恭迎玉皇上帝駐駕 敬獻金牌專案" className="w-full bg-background text-foreground border border-border p-3 rounded-xl outline-none"/></div>
                <div><label className="block text-xs font-bold text-muted-foreground mb-2">專屬獨立匯款帳戶</label><input value={spBankInfo} onChange={e=>setSpBankInfo(e.target.value)} placeholder="例: 玉山銀行 (808) 1234-5678-9012" className="w-full bg-background text-foreground border border-border p-3 rounded-xl outline-none"/></div>
                <div className="md:col-span-2"><label className="block text-xs font-bold text-muted-foreground mb-2">專案詳細緣由與說明</label><textarea value={spDesc} onChange={e=>setSpDesc(e.target.value)} placeholder="說明此專款的用途與歷史意義..." className="w-full bg-background text-foreground border border-border p-3 rounded-xl h-24 outline-none"/></div>
                ...
                <div className="md:col-span-2 flex gap-3">
                  ...
                  {editSpId && <button onClick={() => { setEditSpId(null); setSpTitle(""); setSpDesc(""); setSpBankInfo(""); setSpPreviewUrl(""); setSpOptions([{ title: "隨喜", price: 0 }]); }} className="px-6 bg-background border border-border text-foreground py-4 rounded-xl font-bold tracking-widest text-sm shadow-sm">取消</button>}
                </div>
              </div>

              <div className="space-y-4">
                {spList.map((sp: any) => (
                  <div key={sp.id} className="p-5 bg-card border border-border rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="space-y-2 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground font-mono">方案: {sp.options ? sp.options.map((o:any) => `${o.title}($${o.price})`).join('、') : `未設定`}</p>
                    </div>
                    <div className="flex flex-wrap gap-2 justify-end w-full md:w-auto">
                      ...
                      <button onClick={() => { setEditSpId(sp.id); setSpTitle(sp.title); setSpDesc(sp.description || ""); setSpBankInfo(sp.bank_info || ""); setSpPreviewUrl(sp.image_url || ""); setSpOptions(sp.options || [{title: "隨喜", price: 0}]); }} className="text-xs font-bold bg-muted text-foreground px-4 py-2 rounded-lg">編輯</button>
                      <button onClick={async()=>{if(confirm("確定刪除？")){await supabase.from("special_projects").delete().eq("id", sp.id); fetchData();}}} className="text-xs font-bold bg-red-50 text-red-600 px-4 py-2 rounded-lg">刪除</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {campaignBanks.map((b: any) => (
                  <div key={b.id} className="p-4 bg-card border border-border rounded-xl flex justify-between items-center shadow-sm">
                    ...
                    <button onClick={async()=>{if(confirm("確定刪除？")){await supabase.from("campaign_bank_accounts").delete().eq("id", b.id); fetchData();}}} className="text-xs text-red-500 font-bold hover:text-red-700 px-2 py-1">刪除</button>
                  </div>
                ))}
              </div>
            </section>
        ...
        {activeTab === 'orders' && (
          <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <section className="bg-card text-card-foreground p-6 md:p-8 rounded-[2rem] shadow-sm border border-border space-y-6">
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                      {[
                        ...
                        <button key={tab.id} onClick={() => setSelectedServiceType(tab.id)} className={`px-4 py-2 rounded-xl font-bold text-sm tracking-widest shrink-0 border ${selectedServiceType === tab.id ? 'bg-emerald-50 border-emerald-600 text-emerald-700' : 'bg-card text-muted-foreground border-border'}`}>{tab.label}</button>
                      ))}
                    </div>
                  </div>

                  <div className="w-full xl:w-72 shrink-0">
                    <input type="text" placeholder="搜尋信眾姓名、電話或後五碼" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-background text-foreground border border-border p-2.5 rounded-xl outline-none text-sm"/>
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto max-h-[500px] overflow-y-auto border border-border rounded-xl relative scrollbar-hide">
