...
export default function BurningServicePage() {
  ...
  return (
    <main className="min-h-screen bg-background py-16 px-6 flex items-center justify-center">
      <div className="max-w-4xl w-full bg-card rounded-[2rem] shadow-xl border border-stone-100 overflow-hidden relative">
          ...
          <div className="p-8 md:p-12">
            <form onSubmit={handleAddToCart} className="space-y-12 animate-in fade-in duration-500">
              <div className="space-y-6">
                ...
                {products.length === 0 ? (
                  <p className="text-stone-400 tracking-widest text-sm p-4 bg-muted rounded-xl">目前尚無代燒品項。</p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                    {products.map(p => {
                      const qty = itemQuantities[p.id] || 0;
                      return (
                        <div key={p.id} className={`border-2 rounded-2xl p-5 transition-all flex flex-col justify-between group bg-card ${qty > 0 ? 'border-[#A61D24] shadow-md' : 'border-stone-100 hover:border-stone-300'}`}>
                          {p.image_url && (
                          ...
                          <div>
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="font-bold text-lg text-foreground tracking-wider">{p.title}</h4>
                            </div>
                          ...
                          <div className="flex items-center justify-between mt-4 pt-4 border-t border-stone-100">
                            ...
                            <div className="flex items-center gap-2 bg-muted rounded-full border border-border p-1 shadow-sm">
                              <button 
                                ...
                                disabled={qty === 0}
                                className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-base transition-all ${qty > 0 ? 'bg-card text-stone-600 shadow-sm hover:text-[#A61D24]' : 'text-stone-300 cursor-not-allowed'}`}
                              >
                                -
                              </button>
                              ...
                              <button 
                                type="button" 
                                onClick={() => handleQtyChange(p.id, 1)}
                                className="w-7 h-7 rounded-full bg-card shadow-sm flex items-center justify-center text-stone-600 font-bold text-base hover:text-[#A61D24] transition-all"
                              >
                                +
                              </button>
                            </div>
              ...
              <div className="bg-muted border border-border p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-6">
                <div>
