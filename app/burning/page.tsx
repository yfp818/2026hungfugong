...
export default function BurningServicePage() {
  ...
  useEffect(() => {
    if (selfProfile && !name && selectedContactId === "self") {
      setName(selfProfile.userName); setBirthDate(selfProfile.birthDate); setAddress(selfProfile.address);
    }
  }, [selfProfile]);
  ...
  const handleContactChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedId === "new") {
      setName(""); setBirthDate(""); setAddress("");
      ...
      if (contact) {
        setName(contact.contact_name); setBirthDate(contact.birth_date); setAddress(contact.address);
      }
    }
  };
  ...
  const handleAddToCart = async (e: React.FormEvent) => {
    ...
    updateSharedInfo({ userName: name, birthDate, address });
    const safeId = Date.now().toString() + Math.random().toString(36).substring(2);
    const fallbackPhone = selfProfile?.userPhone || "";

    await addToCart({
      id: safeId, serviceType: "burning", userName: name, userPhone: fallbackPhone,
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-stone-900/60 backdrop-blur-sm transition-all">
            <div className="bg-[#FAF7F0] border-2 border-white p-8 rounded-[2rem] max-w-sm w-full shadow-2xl space-y-8 text-center animate-in zoom-in-95 duration-300 relative overflow-hidden">
              ...
              <div className="flex flex-col gap-3 relative z-10 pt-2">
                ...
                <button onClick={() => { setShowRedirectModal(false); setSelectedContactId("new"); setName(""); setBirthDate(""); setAddress(""); }} className="w-full flex items-center justify-center gap-2 bg-white hover:bg-stone-50 border border-stone-200 hover:border-[#D89F3C] text-stone-600 hover:text-[#1A432D] py-4 rounded-xl font-bold tracking-widest shadow-sm transition-all">
