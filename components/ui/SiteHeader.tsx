...
export default function SiteHeader({ fontClassName }: { fontClassName?: string }) {
  ...
  return (
    <>
      <header className="bg-card/40 backdrop-blur-md sticky top-0 z-40 border-b border-white/60 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className={`${fontClassName} font-bold text-2xl tracking-widest text-slate-800`}>
            皇府宮
          </Link>
          <div className="flex items-center gap-4">
            <Button variant="ghost" className="text-slate-600 hover:bg-card/60 hover:text-slate-900 hidden md:inline-flex tracking-wider">
              您的祈福袋
            </Button>
            <Button variant="ghost" className="text-slate-600 hover:bg-card/60 hover:text-slate-900 tracking-wider" onClick={() => setIsLoginOpen(true)}>
              登入
            </Button>
            <Button className="bg-card/60 text-slate-700 hover:bg-card/90 backdrop-blur-sm rounded-full px-4 text-lg font-bold border border-white/80 shadow-sm" onClick={() => setIsMenuOpen(true)}>
              三
            </Button>
          </div>
        </div>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-card/80 backdrop-blur-xl text-slate-800 p-8 h-full overflow-y-auto animate-in slide-in-from-right border-l border-white/60 shadow-2xl">
      ...
      {isLoginOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/10 backdrop-blur-md p-4 animate-in fade-in">
          <div className="bg-card/70 backdrop-blur-xl border border-white/80 rounded-2xl w-full max-w-md p-8 relative shadow-2xl text-slate-800">
            <button onClick={() => setIsLoginOpen(false)} className="absolute -top-3 -right-3 bg-card text-slate-400 border border-white w-8 h-8 rounded-full flex items-center justify-center hover:text-slate-700 hover:bg-slate-50 shadow-md">&times;</button>
            
            <div className="flex mb-6 bg-card/50 p-1 rounded-lg border border-white/60 shadow-inner">
              <button className="flex-1 bg-card/90 text-slate-800 py-2 rounded-md font-bold tracking-widest shadow-sm">登入</button>
              <button className="flex-1 text-slate-500 py-2 font-medium hover:text-slate-800 tracking-widest">註冊</button>
            </div>
            
            <div className="space-y-4">
              <input type="email" placeholder="請輸入電子郵件" className="w-full bg-card/60 border border-white/80 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200 text-slate-800 placeholder:text-slate-400" />
              <input type="password" placeholder="請輸入密碼" className="w-full bg-card/60 border border-white/80 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-200 text-slate-800 placeholder:text-slate-400" />
